...
afterEach(async () => {
  ...
  delete process.env.TUNARR_AUTOPILOT_RECEIPT_STATS;
...
test.skipIf(!available)(
  ...
  async () => {
    ...
    process.env.TUNARR_AUTOPILOT_RECEIPT_STATS = '1';
...
test.skipIf(!available)(
  'record-only mode leaves the command untouched and captures no stats',
  async () => {
    const dir = await mkdtemp(join(tmpdir(), 'tunarr-receipt-ro-'));
    temporary.push(dir);
    const clip = join(dir, 'clip.mkv');
    execFileSync(FFMPEG, [
      '-f', 'lavfi', '-i', 'testsrc=size=64x64:rate=10:duration=1',
    ...
    process.env.TUNARR_AUTOPILOT_RECEIPT = dir; // STATS deliberately unset
    const process_ = new FfmpegProcess(
      settings,
      'receipt-ro-test',
      ['-hide_banner', '-nostdin', '-y', '-i', clip, '-map', '0:v:0', '-an', '-c:v', 'libx264', '-f', 'mpegts', join(dir, 'out.ts')],
      dir,
    );
    await new Promise<void>((resolve) => {
      process_.once('exit', () => resolve());
    ...
    while (Date.now() < deadline) {
      try {
        lines = (await readFile(receipt, 'utf-8')).trim().split('\n').filter(Boolean).map((line) => JSON.parse(line) as Record<string, unknown>);
        if (lines.some((line) => line.kind === 'completion')) break;
      } catch {
        // not written yet
      }
    ...
    const invocation = lines.find((line) => line.kind === 'invocation');
    const completion = lines.find((line) => line.kind === 'completion');
    expect(invocation).toMatchObject({ sourceId: clip, statsFile: null });
    expect((invocation!.args as string[]).some((arg) => arg.startsWith('-stats_mux_pre'))).toBe(false);
    expect(completion).toMatchObject({ exitCode: 0, statsRows: 0 });
  },
  30_000,
);
